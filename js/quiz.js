...
const EQ_Quiz = (() => {
  ...
  /* ─── INIT ─── */
  const initWithParams = ({ subject, mode } = {}) => {
    state.subject = subject || 'matematica';
    state.mode    = mode    || 'quiz';
    resetState();
    startQuiz();
  };
  ...
  const init = () => {
    ...
    initWithParams({ subject: params.get('subject'), mode: params.get('mode') });
  };
  ...
  const handleResult = (isCorrect, q) => {
    if (isCorrect) {
      ...
      state.wrong++;
      EQ_EventBus.emit('answer:wrong', { question: q.q, explanation: q.explanation });
    }
    ...
    container.innerHTML = `
      <div class="quiz-results animate-scale-in">
        ...
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-top:24px">
          ...
          <button class="btn btn-secondary btn-lg" onclick="window.location.href='app.html'">Dashboard 🏠</button>
        </div>
      </div>
    `;
  ...
  return { init, initWithParams, destroy, shuffle };
})();
